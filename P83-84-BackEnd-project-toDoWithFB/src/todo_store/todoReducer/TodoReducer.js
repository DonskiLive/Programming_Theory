...
const todoReducer = createSlice({
    ...
    reducers:{
        ...
        addTodo: (state, {payload}) =>{
            state.todos.push({...payload, status:false})
        },
...
export const addTodoAction = (title, uid, img)=>{
    return async dispatch =>{
        dispatch(startLoading())
        try{
            const date = Date.now()
            const response = await addTodoItem(title, uid,img, date) // db
            console.log(response)
            dispatch(addTodo({title, img, date}))  //state
...
export const removeTodoAction = (index, todos, uid, img) =>{
    return async dispatch =>{
        dispatch(startLoading())
        try{
            const newTodos = [...todos]
            newTodos.splice(index,1)
            await changeTodos(newTodos, uid, img)
