...
function userClickHandler (e){
  left.innerHTML = '';
  right.innerHTML = '';
  const userId = +e.currentTarget.id.split('_')[1]  //user_1
  console.log(userId);
  left.append(getAlbumsByIserId(userId))

}
const getAlbumsByIserId = (userId)=>{
  const element = document.createElement('div');
  const userAlbums = albums.filter(el => el.userId === userId);
  if(!userAlbums.length){
    element.innerHTML = 'No Albums';
    return element
  }
  userAlbums.forEach(el => {
...
function albumClickHandler(e){
  right.innerHTML = '';
  const albumId = +e.currentTarget.id.split('_')[1]  //album_1
  right.append(getPhotoByAlbomId(albumId))
}

function getPhotoByAlbomId(id){
  const element = document.createElement('div');
  const photosByAlbumId = photos.filter(photo => photo.albumId === id);
  if(!photosByAlbumId.length){
    element.innerHTML = 'no photos';
    return element
  }
  photosByAlbumId.forEach(photo =>{
